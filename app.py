...
def plot_array_and_source(array_size=8, source_distance=2.0, source_angle=(1/4)*np.pi, microphone_spacing=0.2):
    ...
    lim = max(array_size * microphone_spacing / 2, source_distance) * 1.05
...
def calculate_delays(microphone_positions, source_distance, source_angle, frequency):
    source_x = source_distance * np.sin(source_angle)
    source_y = source_distance * np.cos(source_angle)

    distances = np.sqrt((microphone_positions - source_x)**2 + source_y**2)
...
def plot_waveforms(frequency, sampling_rate=100000, array_size=8, microphone_spacing=0.1, source_distance=2.0, source_angle= (1/4)*np.pi):
    microphone_positions = initialize_array(array_size, microphone_spacing)
    phase_shifts, delays = calculate_delays(microphone_positions, source_distance, source_angle, frequency)
...
if __name__ == "__main__":
    ...
    plot_array_and_source(microphone_spacing=0.1, source_angle=1*np.pi/4)
    plot_waveforms(frequency=100, microphone_spacing=0.1)
